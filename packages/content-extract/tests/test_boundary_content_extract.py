...
policy leaked into the mechanism. Keep it green. Importing ``convert_md`` is fine:
it is a shelf sibling, not a consumer domain.
"""
...
def test_imports_no_consumer() -> None:
    pkg_dir = Path(content_extract.__file__).parent
    offenders: list[str] = []
    ...
    assert offenders == [], f"content_extract must not import a consumer: {offenders}"
