"""Boundary guard: llm-cache must not import a host framework (kept reusable)."""
...
_SRC = Path(__file__).resolve().parents[1] / "src" / "llm_cache"


def test_llm_cache_imports_no_host() -> None:
    ...
    assert not offenders, f"llm-cache must stay host-agnostic; found: {offenders}"
