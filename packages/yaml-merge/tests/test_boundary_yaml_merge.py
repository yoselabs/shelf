...
If this fails, a consumer's types or policy leaked into the mechanism — the merge
has stopped being reusable. Keep it green.
"""
...
def test_imports_no_consumer() -> None:
    pkg_dir = Path(yaml_merge.__file__).parent
    offenders: list[str] = []
    ...
    assert offenders == [], f"yaml_merge must not import a consumer: {offenders}"
